...
10. Run 4-bit Shor's ECDLP circuit on real quantum hardware via Classiq (IBM Quantum). Add `run_hardware()` function to `shor_ecdlp_classiq.py` that configures `IBMBackendPreferences`, executes with sufficient shots, and post-processes the result. Verify d=6 is recovered from real hardware measurement data.
